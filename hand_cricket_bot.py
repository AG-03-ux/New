...
USE_WEBHOOK = int(os.getenv("USE_WEBHOOK", "1"))
...
class RateLimiter:
    ...
    def get_wait_time(self, user_id: int, action_type: str = 'default') -> float:
        ...
        return user_queue[0] + window - time.time()
...
def db_init():
    ...
    with get_db_connection() as db:
        ...
        logger.info("Basic database tables initialized")

def init_tournament_db():
    """Initialize tournament-related database tables safely"""
    with get_db_connection() as db:
        try:
            ...
            # Create other tournament tables
            db.execute("""
...
def enhanced_process_ball_v2(chat_id: int, user_value: int, user_id: int):
    ...
    try:
        ...
        if check_innings_end(game_state.data):
            end_innings_or_match_v2(game_state, user_id)
    ...
    if game_state.data['innings'] == 1:
        if game_state.data['batting'] == 'player':
            game_state.update(
                ...
                balls_in_over=0,
                overs_bowled=0,
                is_powerplay=True if game_state.data['powerplay_overs'] > 0 else False
            )
        else:
            game_state.update(
...
def kb_main_menu() -> types.InlineKeyboardMarkup:
    ...
    kb.add(
        types.InlineKeyboardButton("🏆 Tournaments", callback_data="tournament"),
...
# Tournament keyboards - simplified
def kb_tournament_main() -> types.InlineKeyboardMarkup:
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(types.InlineKeyboardButton("🚧 Coming Soon", callback_data="tournament_soon"))
...
def show_achievements(chat_id: int, user_id: int):
    try:
        achievements_text = (
            f"🏅 <b>Achievements</b>\n\n"
            f"Complete matches to unlock achievements!\n\n"
            f"Available achievements:\n"
            f"🏆 First Victory - Win your first match\n"
            f"💯 Century Maker - Score 100 runs\n"
            f"🎩 Hat-trick Hero - Take 3 wickets in a row\n"
            f"🔥 Consistent Player - Win 5 matches in a row\n"
            f"🚀 Big Hitter - Hit 50 sixes\n\n"
            f"Keep playing to unlock more!"
        )
        bot.send_message(chat_id, achievements_text)
    except Exception as e:
        logger.error(f"Error showing achievements: {e}")
        bot.send_message(chat_id, "❌ Error loading achievements.")
...
def handle_ball_input(message: types.Message):
    try:
        ...
        if isinstance(result, dict):
            ...
            show_live_score(message.chat.id, result['game_state'])
        else:
...
@bot.message_handler(func=lambda message: message.text and "📊 score" in message.text.lower())
...
@bot.message_handler(func=lambda message: message.text and "🏳️ forfeit" in message.text.lower())
...
def handle_callback(call: types.CallbackQuery):
    try:
        ...
        if not rate_limiter.is_allowed(user_id, 'callback'):
            wait_time = rate_limiter.get_wait_time(user_id, 'callback')
            bot.answer_callback_query(call.id, f"Please wait {wait_time:.1f} seconds")
            return
        ...
        # Tournaments
            ...
            bot.edit_message_text(
                ...
                chat_id, call.message.message_id,
                reply_markup=kb_tournament_main()
            )
        ...
        # Toss callbacks
            ...
            handle_toss_result(chat_id, "heads", user_id)
            bot.answer_callback_query(call.id, "You chose Heads!")
        
        elif call.data == "toss_tails":
            handle_toss_result(chat_id, "tails", user_id)
            bot.answer_callback_query(call.id, "You chose Tails!")
        ...
        # Post match actions
            ...
            # This would show the last match summary
...
def handle_toss_result(chat_id: int, user_choice: str, user_id: int):
    try:
        ...
        if user_choice == toss_result:
            ...
            # Start the match with bot's choice
